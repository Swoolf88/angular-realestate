...
export class BookListComponent implements OnInit {
  ...
  listBooks(){
    ...
    this.searchMode1 = this._activatedRoute.snapshot.paramMap.has('keyword1');
    if(this.searchMode){
      //do search work
      this.handleSearchBooks();
    }if(this.searchMode1){
        //do search work
        this.handleSearchDescriptionBooks();
    }else {
  ...
  handleSearchDescriptionBooks(){
    const keyword1: string = this._activatedRoute.snapshot.paramMap.get('keyword1');

    this._bookService.searchDescriptionBooks(keyword1,
                                  this.currentPage - 1,
                                  this.pageSize)
                                  .subscribe(this.processResult());
  }
